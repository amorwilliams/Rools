...
| 用途 | **仅参考** Eurorack 电源、PCM3060 Codec、±10V 音频调理、jack 区 |
| 非目标 | 不照搬整板：Rools 无 Patch 前端面板、无 Patch 控件拓扑 |
...
|------|------------|------------|
| MCU | Daisy Seed | 同 |
| Codec | PCM3060 | 同（BOM 待定） |
| 电源 | Eurorack → 稳压 | Buck +5V（ADR-011） |
| 显示 | 无 | ST7735 1.77" SPI |
| CV | Patch 自有 | 4 In + MCP4728 ×4 Out |
...
2. **PCM3060** + I2S/I2C 到 Seed
3. **音频 In/Out** 运放 ±10V 调理
4. **PJ398/Thonkiconn** jack + normalled（IN_R→IN_L，ADR-010）
