...
| DAC8565 + OPA4171 CV Out | ~10–20 mA |
...
### 参考器件（KiCad Power 页 Rev 0.2）

| 功能 | 器件 / 参数 |
|------|-------------|
| +12V → +5V | **TPS54302** Buck；FB R4=73.2 k / R2=10 k → **5.0 V** |
| +5V → +3.3V | **AP2112K-3.3** LDO → `+3V3_D` |
| DAC 模拟 | `+3V3_D` → 磁珠 → **`+3V3_DAC`**（DAC8565 AVDD） |
| 掉电 hold-up | **D3 SS34**：`+12V` → `+12V_SAFE`；**C36+C37** 各 1000 µF（共 ~2000 µF） |
| 掉电预警 | **LM393 U10A**：+12V 分压 vs `+3V3_D` → `EXTI_PWR` → Seed D28；阈值 ~10.6 V |
| USB VBUS | 限流 High-side switch + ESD |

掉电保存软件见 [ADR-015](ADR-015-settings-persistence.md)；Flush 前先 GPIO 关 ST7735 背光以节省 hold-up 电流。
