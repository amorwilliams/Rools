...
面包板**不实现**。Seed 页 **D28 / pin 35** 已留 net **`EXTI_PWR`**；比较器、保持电容等前端电路 **原理图待画**。

| 硬件 | 作用 |
|------|------|
| +5V 保持电容 | 拔电后维持数十 ms 供 MCU 写 Flash |
| 比较器监测 +12V 或 +5V 跌落 | 提前触发，不等电压归零 |
| 比较器 → Seed D28（`EXTI_PWR`） | 下降沿最高优先级 ISR |
...
- `pins.h`：`D28` / `EXTI_PWR` 注释（`kPwrFail` 待 M2 启用）
...
- [ ] EXTI 引脚：**D28**（`EXTI_PWR`，Seed 页已接）；勿 D13（Exp MIDI TX）
- [ ] 掉电前端：比较器 + 保持电容原理图（**待画**）
